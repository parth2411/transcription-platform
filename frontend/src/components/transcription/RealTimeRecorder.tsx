...
export function RealTimeRecorder({ onTranscriptionComplete }: RealTimeRecorderProps) {
  ...
  // Enhanced real-time processing refs
  ...
  const startRecording = async () => {
    try {
      ...
      // Enhanced data handling for real-time processing
      mediaRecorderRef.current.ondataavailable = (event) => {
        if (event.data.size > 0) {
          audioChunksRef.current.push(event.data)
          transcriptionQueueRef.current.push(event.data)
        }
      }
      ...
      // Start recording with frequent data collection for real-time processing
      ...
      // Enhanced real-time transcription processing
      chunkIntervalRef.current = setInterval(async () => {
        await processRealTimeQueue()
      }, 2000) // Process every 2 seconds for more frequent updates
  ...
  const processRealTimeQueue = async () => {
    if (transcriptionQueueRef.current.length === 0 || isTranscribing) {
      return
    }

    try {
      setIsTranscribing(true)
      setPendingChunks(transcriptionQueueRef.current.length)

      // Get accumulated audio chunks (last 6 seconds worth)
      const recentChunks = transcriptionQueueRef.current.slice(-12) // Last 12 chunks (6 seconds)
      const chunkBlob = new Blob(recentChunks, { type: 'audio/webm' })

      // Only process if we have substantial audio data
      if (chunkBlob.size < 1000) { // Less than 1KB
      ...
      formData.append('continuous', 'true') // Flag for continuous transcription
      formData.append('timestamp', Date.now().toString())

      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/transcriptions/realtime-stream`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.text && result.text.trim()) {
          // Handle both partial and final transcriptions
          if (result.is_final) {
            // Final transcription - append to complete text
            setRealTimeText(prev => {
              const newText = prev + ' ' + result.text
              partialTranscriptRef.current = newText
              return newText
            })
            // Clear processed chunks to avoid reprocessing
            transcriptionQueueRef.current = transcriptionQueueRef.current.slice(-4)
          } else {
            // Partial transcription - show as temporary
            const baseText = partialTranscriptRef.current
            setRealTimeText(baseText + ' ' + result.text)
          }
        }
      }
    } catch (error) {
  ...
  const processCompleteRecording = async (audioBlob: Blob) => {
    try {
      ...
      if (response.ok) {
        ...
        setFinalTranscription(result.text || realTimeText)
        ...
        setFinalTranscription(realTimeText)
      }
    } catch (error) {
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-600">
          Record with live transcription and automatic summarization
        </p>
      </div>
      ...
      {!isRecording && !completedResult && (
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="knowledge-base">Add to Knowledge Base</Label>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-center space-x-4">
        ...
        {(realTimeText || finalTranscription) && !isRecording && (
          <Button
      ...
      {(isRecording || realTimeText) && (
        <div className="space-y-3">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 min-h-[120px] border border-blue-200">
        ...
        <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
          <div className="flex items-center justify-center space-x-3">
            ...
            <span className="text-blue-700 font-medium">
              Processing complete recording and generating summary...
            </span>
          </div>
        </div>
      )}
          ...
          <div className="space-y-3">
            ...
            <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
              <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-wrap">
                {finalTranscription || "No transcription available."}
              </p>
            </div>
          </div>
